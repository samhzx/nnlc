...
`NNLC_Trainer.exe` 是 Tkinter 操作界面。双击后选择 reallog 目录、输出目录，填写车型；路线阈值保持“自动推荐”即可。训练模式仅使用 CPU，可选“CPU 标准模式”或“CPU 低内存模式”，后者使用更小批次来降低内存峰值。训练中间 CSV、覆盖度图、验证图和最终车型 JSON 都会写到输出目录。
