...
Windows 发布包提供 `NNLC_Trainer.exe` 图形界面，适合不想使用命令行的场景。窗口采用“目录设置 + 训练参数 + 运行日志”的布局，启动后按以下顺序填写：
...
5. 训练模式按数据规模选择。项目固定使用 CPU，不会启用 GPU：
   ...
   - “CPU 流式低内存模式”：逐个 rlog 文件完整排序，之后对评分、剪枝、覆盖度统计、验证抽样和 Julia 预处理采用有界内存处理，适合 16GB 内存电脑和上亿行 CSV。
6. GUI 始终使用“容错模式（跳过损坏日志）”。无法读取的文件会写入运行日志，训练前应确认剩余数据量足够。
7. 点击“开始训练”后，覆盖度图和完整中间 CSV 始终保留，便于检查数据和排查失败原因。GUI 不提供关闭这两个行为的复选框；如需清理中间 CSV，请改用命令行参数。

GUI 会自动记住上次选择的车型和训练模式。输入目录、输出目录和路线阈值需要按每次任务重新确认。
运行日志区域底部提供“打开输出目录”“清空日志”“保存日志”和耗时显示；训练结束后可以清空或保存当前窗口日志，保存文件默认写入 EXE 所在目录。
...
- `NNLC_Trainer.exe`：包含 Python、项目代码和训练脚本。GUI 顶部标题区和底部状态栏显示当前版本；打包后的 Windows 程序启动后会静默检查一次更新。
- `NNLC_Julia_Runtime-windows-x64-v1.zip`：包含固定的 Julia 1.10.11、CPU 训练依赖和 depot，长期发布在 `julia-runtime-v1` GitHub Release 中。

首次使用时，将 EXE 和环境 ZIP 放在同一个目录并双击 EXE。程序会验证 ZIP，并自动解压为 `NNLC_Runtime`；准备完成后即可训练。只要 `NNLC_Runtime` 完整且版本匹配，ZIP 可以删除。日常更新会把带版本号的新 EXE 下载到当前程序目录，例如 `NNLC_Trainer-<version>-windows-x64.exe`，不会覆盖正在运行的文件。关闭当前程序后运行新 EXE 即可，原来的 Julia 环境可以继续使用。
...
CSV 提取器逐个处理 rlog 文件：每个文件完整读取并按 `logMonoTime` 稳定排序，排序完成后仍将提取结果增量写入 CSV，不会把全部 rlog 文件同时保存在内存中。zstd/bz2 日志会先解压到临时文件，默认优先放在程序（exe）所在目录；如果该目录不可写，会自动回退到系统临时目录。也可以通过 `NNLC_TEMP_DIR` 环境变量指定临时目录，因此需要保证该磁盘有足够空间。由于单个 `.zst` 文件通常不超过 20 MB，完整排序可以覆盖文件内的所有乱序事件，不会因固定窗口而丢弃数据。Parquet 输出仍需由 pandas 读取最终 DataFrame，超大数据集建议优先使用 CSV 或分批处理。
...
从设备按日期拷贝的目录也可以直接选择最外层数据目录：

```
./data/
  0828/
    000000a0--68a50e9c1b--0/rlog.zst
    000000a0--68a50e9c1b--1/rlog.zst
    ...
```

最后的 `--0`、`--1` 是每分钟分段号；程序会将上述分段统一识别为路线
`000000a0--68a50e9c1b`，日期目录不会被当作路线 ID。
